...
- [x] **2.15** `sensor_msgs/BatteryState` (voltage + summed current + percentage) and/or `DiagnosticArray` aggregation (review §6.1, feature §10).
...
**Status 2026-07-24: ALL items implemented, deployed and verified (commits: driver `2e52120`, bridge `78b514c`, mission `27cd108`, OLED `176638c`, web UI `c2ab136`). Implementation refinements vs. the original design (recorded so the docs match reality):**
- **Standing permission implementation:** the driver's `motors_enabled` now **defaults TRUE** (deviation from the earlier "driver default false, backend asserts" note). Persistence lives in the **bridge's param_control overrides file** — a new ParamManager behavior re-applies stored overrides whenever a target node comes online, so an OFF preference survives Pi reboots. The driver arms only on an actual false→true parameter change, so idempotent re-applies never beep/arm; boot never arms (no activation edge) — the first drive command arms silently.
- **3.2c (web toggle) went through the existing param_control path** (`ros2/mowparams/cmd`) — no new set-channels manager, no mosquitto ACL change, no FastAPI backend involvement. MotorsControl card on Drive + Status pages with double-confirm and a freewheel warning.
- **3.5 needed NO new code:** the mission system already had a battery gate (`mow_battery_low_voltage` 35 V default, debounce, pause with reason "battery", manual resume, surfaced in web UI incl. AlertBanner and on the OLED mission state). A duplicate percentage gate was deliberately not added; `battery_state` percentage now feeds the displays.
- **Newton k** is a per-browser localStorage setting (Settings page), default 6 N/A until the spring-scale calibration.
- **2.15 verified live:** battery_state on MQTT (42.2 V, 100 %, discharging). **DC-current sign still unresolved after an in-air drive test (2026-07-24)**: in-air draw (~0.2 A) is the same magnitude as the sensor offsets (idle −0.30 A summed vs spinning −0.27 A) — needs a LOADED ground drive (1-3 A real current) to settle; BatteryState keeps the positive=discharge assumption until then.
- **In-air drive test 2026-07-24 (wheels off ground, user supervising):** silent re-arm verified (standby → cmd_vel → armed → drove → clean stop); **cmd echo verified live** (echo locked to the commanded value within int16 quantization for the whole drive, 0 after stop — task 3.6 exercised for real); both wheel positions accumulated IDENTICALLY (+27.93 rad) driving forward — right-wheel sign fix validated on hardware.
- **URDF fix (2026-07-24, commit `6fe73b3`):** right wheel joint axis was −Y, compensating the old driver's inverted right position — made the right wheel render backwards in Foxglove once the driver was fixed. Now +Y like the left; visualization-only. Gotcha: xacro comments must not contain colon+space (launch YAML-parses robot_description).
- **Known refinement (follow-up):** right-wheel `iq` arrives sign-mirrored (negative driving forward — firmware passes raw q-axis current without mirror compensation) and noisy at 10 Hz; before trusting the newton force display, consider driver-side negation of iq_r + light smoothing.
- **Power manager (3.1):** acts only during its startup window with retries/backoff, then permanently stands down once the board has been seen connected — a deliberately powered-off board is never re-pulsed. Verified: with the board already on it stood down without pulsing; a full boot-with-board-off cycle is still worth one observation.
- **Mission gate (3.2) verified live:** start with master switch OFF → `success=False, "drive motors master switch is OFF - turn Motors ON (web UI Drive page) and start again"`.
...
**DECIDED 2026-07-24: standing permission** — `motors_enabled` stays true as a persistent master switch, with a web-UI toggle + visible status. Manual driving "just works": any drive input silently arms, 2-min auto-disable returns to standby. Viable because every twist_mux source is deliberate, deadman'd (0.5 s), and behind the F31 e-stop lock.

Design consequences:
- **Driver code default stays `false` (safe); the web backend is the persistence layer**: it stores the user's preference (default ON) and asserts it via the parameter service once the stack is up. Turning the switch off for maintenance therefore survives reboots.
- **No arming edge at boot**: `prev_allowed_` is initialized from the parameter at activation, so a boot-time `true` does NOT arm (no beeps / 2-min holding torque per boot) — the first drive command arms silently. An explicit set-to-true from the web toggle still arms immediately with beep feedback (`arm_edge`, `0319989`).
- **Two states shown in the UI**: the toggle mirrors a new latched `hoverboard/motors_allowed` topic (the master switch), and an "armed" indicator mirrors the existing `hoverboard/motors_enabled` topic (motors live right now). With standing permission these differ most of the time — allowed=true, armed only while driving.
- **BT checks, never sets**: missions verify motors are allowed/armed (condition + wait-for-armed) and fail with a clear reason if the master switch is off — a deliberate maintenance-off must not be silently overridden by a scheduled mission. No disarm-at-dock action needed (auto-disable covers it).
- **Plumbing**: the MQTT bridge maps a set-channel to the parameter service and forwards both status topics to MQTT; the earlier `arm_request` Bool-topic idea is dropped (the rclpy bridge calls the service directly). A joystick arm button is optional/deferred — with standing permission, driving is the arming action.

- [x] **3.1** Boot orchestration: if `hoverboard/connected` false after grace period → single `hoverBtnR1_pulse` → wait for connected (never pulse blindly — it's a toggle; feature §7).
- [ ] **3.2** BT: **check, don't set** (per decided design) — condition node on `hoverboard/motors_allowed` + wait-for-`hoverboard/motors_enabled` before driving; mission fails with clear reason if the master switch is off. No disarm-at-dock action (auto-disable covers it).
- [x] **3.2b — driver part (rides with the 2.15 build):** (a) initialize `prev_allowed_` from the parameter at activation → no arming edge at boot; (b) new latched on-change topic `hoverboard/motors_allowed` mirroring the parameter.
- [x] **3.2c — web part (batch with 3.3/3.4):** MQTT bridge set-channel → parameter service; forward `motors_allowed` + `motors_enabled` to MQTT; web-UI master toggle + armed indicator; backend persists the preference (default ON) and asserts it at startup.
- [x] **3.3** Web-UI settings field for `auto_disable_timeout` (backend → SetParameters on `hoverboard_driver_node`).
- [ ] **3.4** Surface in web UI: motor state, battery voltage + percentage (from 2.15 battery_state), motor error codes (plain-language).
- [x] **3.4c — OLED panel (DECIDED IN 2026-07-24):** extend `mowbot_oled_ui` with (a) motors state line — armed / standby / master-switch-off (from `motors_enabled` + `motors_allowed`); (b) motor error display (hall disconnected / short / blocked) so faults are readable standing at the robot; (c) battery percentage unified from `battery_state` instead of the panel's own local voltage→% computation (panel already subscribes to `battery_voltage`, which stays available as fallback).
...
- [x] **3.5** Low battery during mission — **DECIDED 2026-07-24: notify-only v1** (return-to-home is not implemented yet, dock even less): configurable threshold on `battery_state` percentage → cleanly end the mowing mission (blades off, robot stops in place; motors reach standby via the 2-min auto-disable; board stays on streaming telemetry — that's the always-on feature working, with the firmware's `BAT_DEAD` poweroff as the last line) + **notification** via MQTT → web-UI alert + OLED panel message. Upgrade path: swap "end mission" for "return to dock" once docking lands.
- [x] **3.6 — cmd echo check (DECIDED IN 2026-07-24, rides with the driver build):** driver compares each sent steer/speed against the firmware's `cmd1`/`cmd2` echo from the next feedback frame (identity mapping in this config, one-frame delay). Expose echo on the debug topics (10 Hz, `publish_debug`-gated); throttled WARN with both values when sent vs echoed disagree beyond tolerance for several consecutive cycles while armed and connected. Catches silent command distortion (skewed input mapping, unexpected firmware clamping, future protocol drift). Driver-only, no firmware/protocol change.
- [ ] **Deferred ideas:** IMU level-check before mid-mission disarm (feature §8) — **excluded from this pass by user decision**.
