...
- [ ] **3.2b — driver part (rides with the 2.15 build):** (a) initialize `prev_allowed_` from the parameter at activation → no arming edge at boot; (b) new latched on-change topic `hoverboard/motors_allowed` mirroring the parameter.
- [ ] **3.2c — web part (batch with 3.3/3.4):** MQTT bridge set-channel → parameter service; forward `motors_allowed` + `motors_enabled` to MQTT; web-UI master toggle + armed indicator; backend persists the preference (default ON) and asserts it at startup.
